...
- ✅ Same-origin API proxy (2026-09-26, DEPLOYED, cache `v=26`) — `functions/[[path]].js` `proxyApi()` forwards `/api/*` to Fly; `config.js` `API_BASE = ''`. Fixes Googlebot rendering a full-screen "Failed to fetch" error (GSC Live Test), a likely cause of 0/856 pages indexed. Verified live. Next: GSC Live Test re-check + re-request validation. Every launch's source PDFs + a per-project reference `.md` archived in `bto_launches_info/`. See `memory-bank/activeContext.md` for the full backfill writeup and the `seed-bto-launch` skill for the curation runbook. (240 tests pass)
