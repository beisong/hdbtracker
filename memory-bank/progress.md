...
## What Works
- ✅ BTO Launches — `bto_projects` table (dual-seeded like `hdb_block_coords`) + `/api/bto/launches`, `/api/bto/projects`, `/api/bto/project-overview`; BTO project names searchable from the main search bar (exact-match resolve + autocomplete) with a project page showing flats/prices and a live BTO-vs-nearby-resale comparison; `/bto` + `/bto/<slug>` routes; SEO metadata + sitemap. **121 launches, spanning April 2001 → June 2026, zero known gaps** — full historical backfill of HDB's entire BTO history complete. Through 2010-06 committed/pushed (`e5a0709`); the last 5 launches (2010-07/08/09, 2011-05, 2011-07) are complete locally but not yet committed — see `memory-bank/activeContext.md`. Every launch's source PDFs + a per-project reference `.md` archived in `bto_launches_info/`. See `memory-bank/activeContext.md` for the full backfill writeup and the `seed-bto-launch` skill for the curation runbook. (240 tests pass)
